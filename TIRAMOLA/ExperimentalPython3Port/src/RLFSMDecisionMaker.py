...
import fuzz, logging, math, time, _thread, random, itertools
...
class RLFSMDecisionMaker():
    def __init__(self, eucacluster, NoSQLCluster, VmMonitor):
        ...
        if os.stat(self.measurementsFile).st_size == 0:
            ...
            next(meas) # Skip the first line with the headers of the columns
            for line in meas:
    ...
    def addMeasurement(self, metrics, writeFile=False):
        ...
        if self.measurementsPolicy.startswith('average'):
            if metrics[0] not in self.sumMetrics:
        ...
        if metrics[0] not in self.memory:
...
#            if sliced_data == None:   
            ...
            if len(label) > 0:
                ...
                max_meas_cluster = max(iter(list(num_of_meas.items())), key=operator.itemgetter(1))[0]
...
#                self.my_logger.debug("TAKEDECISION Vs: "+ str(Vs) +", max V = "+ str(max(Vs)) +" V["+str(s)+"] "+ str(V[s]))
        ...
        # Find the max V
        self.nextState = str(max(iter(list(V.items())), key=operator.itemgetter(1))[0])
...
#            fp.close()
        ...
        if self.debug :
            ...
            if self.acted[len(self.acted)-1] == "done" :
                ...
                _thread.start_new_thread(self.polManager.act, (action, self.acted, self.currentState, self.nextState))
