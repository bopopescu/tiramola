...
import logging.handlers, _thread, threading, time, sys
import Utils
...
#                                self.my_logger.debug("Action ADD NODE triggered by: " + thresname + " = " +  host[thresname])
        for host in list(allmetrics.values()):
            votes = []
            for thresname, thresvalue in list(self.utils.thresholds_add.items()):
                splits = thresvalue.split("_")
                if (splits[0] == "high"):
                    if thresname in host:
                        if splits[1].startswith("%"):
                            ## If the measure is percentile, then evaluate with the total metric from ganglia
                            val = splits[1].replace("%","")
                            if thresname.replace(thresname.split("_")[1],"total") in host:
                                if (float(host[thresname])/float(host[thresname.replace(thresname.split("_")[1],"total")]) * 100) > float(val):
#                                    action = "add"
...
#            print "votes=", votes
        ...
        votes = []
        for thresname, thresvalue in list(self.utils.thresholds_remove.items()):
            splits = thresvalue.split("_")
            if (splits[0] == "high"):
                for host in list(allmetrics.values()):
                    if thresname in host:
...
#                action = "add"
        ...
        if self.acted[len(self.acted)-1] == "done" :
            ...
            _thread.start_new_thread(self.polManager.act, (action,self.acted))
