...
#            self.quorum = host_template+"master,"+host_template+str((int(self.utils.initial_cluster_size)/2))+","+host_template+str(int(self.utils.initial_cluster_size)-1)
        ...
        for line in fileinput.FileInput(hbase_site,inplace=1):
            line = line.replace("ZK_QUORUM_IPs", self.quorum )
            print line
...
#            print stdout.readlines()
        ...
        for (clusterkey, clusternode) in self.cluster.items():
            for line in fileinput.FileInput(known_hosts_name,inplace=1):
                line = line.replace(clusternode.public_dns_name, clusterkey)
                print line
...
#        ## make the quorum
#        for line in fileinput.FileInput(hbase_site,inplace=1):
#            line = line.replace("ZK_QUORUM_IPs", self.host_template+"master,"+self.host_template+str((len(self.cluster.keys())/2)+1)+","+self.host_template+str(len(self.cluster.keys()-1)))
#            print line
#            
#        ## make the balancing
#        for line in fileinput.FileInput(hbase_site,inplace=1):
